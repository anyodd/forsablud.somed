...
<section class="content px-0">
  <div class="container-fluid"><br>
    <div class="row">
      <div class="col-md-12">
          <a href="#" class="btn btn-sm btn-primary disabled">Pendapatan Rutin</a>
          <a href="{{route('realisasitl.bulan',date('m'))}}" class="btn btn-sm btn-primary">Piutang</a>
              ...
              <table class="table table-sm table-bordered table-hover mb-0" id="example" width="100%" cellspacing="0">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="text-right">{{ number_format($realisasi->piutang,2,',','.') }}</td>                      
                    <td class="text-right">{{ number_format($realisasi->realisasi,2,',','.') }}</td>                      
                    <td class="text-center">
                      <a href= "{{ route('realisasi.tambah',$realisasi->id_bp) }}" class="btn btn-sm btn-success" title="Realisasi" onclick="deleteItems()">
                        <i class="fas fa-money-check"></i>
                      </a>
                      <a href="{{ route('tbp_pdf', $realisasi->id_bp)}}" class="btn btn-sm btn-primary" target="_blank" style="float: right;" title="Preview/Cetak">
...
<script>
  ...
  function Mymonth()
  {
    var slug = $('#bulan').val();
    console.log(slug);
    var url = '{{ route("realisasi.bulan", ":slug") }}';
