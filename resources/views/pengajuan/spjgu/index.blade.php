...
<section class="content px-0">
  <div class="container-fluid">
    <div class="row">
      {{-- <div class="col-md-12">
          <a href="{{route('spjgu.index')}}" class="btn btn-primary disabled">Tahun Berjalan</a>
          <a href="{{route('spjguutang.index')}}" class="btn btn-primary">Tahun Lalu</a>
      </div> --}}
    </div>  
      ...
      <div class="col">
        <div class="card shadow-lg mt-2">
          <div class="card-header bg-info py-2">
            <h5 class="card-title font-weight-bold">Data GU (Ganti Uang)</h5> 
          </div>
            <div class="card-body px-2 py-2">
            <a href="{{ route('spjgu.create') }}">
            <button class="btn btn-sm btn-warning">
              <i class="fas fa-plus-circle pr-1"></i>
              Tambah GU (Ganti Uang)
            </button>
            </a>
              ...
              <table class="table table-sm table-bordered table-hover mb-0" id="example" width="100%" cellspacing="0">
                ...
                <?php $no = 0;?>
                  ...
                  <tr>
                    ...
                    <td class="align-top">
                      <div class="row justify-content-center" >
                        {{-- <form action="{{route('spjgu.viewtambahrincian',$spjgu->id)}}" method="get" class="">
                          <button type="submit" class="btn btn-sm btn-info file-alt" name="submit" title="Tambah Rincian">
                            <i class="fas fa-plus-circle"></i>
                          </button>
                        </form>
                        <form action="{{route('spjgu.rincian',$spjgu->id)}}" method="get" class="">
                          <button type="submit" class="btn btn-sm btn-success file-alt" name="submit" title="List Rincian">
                            <i class="fas fa-file-alt"></i>
                          </button>
                        </form> --}}

                        <a href="{{ route('sppdgu_pdf', $spjgu->id)}}" class="btn btn-sm btn-primary" target="_blank" style="float: right;" title="Preview/Cetak">
                        ...
                        @if ($spjgu->Tag_v == 0)
                          <button class="btn btn-sm btn-warning file-alt mx-1" data-toggle="modal" data-target="#modal_log" id="catatan" data-row="{{$spjgu->id}}" title="Catatan">
                            <i class="fas fa-envelope"></i>
                          </button>
                        @else
                          <button class="btn btn-sm btn-warning file-alt mx-1" data-toggle="modal" data-target="#modal_log" id="catatan" data-row="{{$spjgu->id}}" title="Catatan" disabled>
                            <i class="fas fa-envelope"></i>
                          </button>
                        @endif
                        @if ($spjgu->Tag_v == 0)
                        <button class="btn btn-sm btn-primary file-alt mx-1" data-toggle="modal" data-target="#modal_verif{{$spjgu->id}}" title="verfikasi">
                          <i class="fas fa-file-invoice"></i>
                        </button>
                        @else
                        <button class="btn btn-sm btn-primary file-alt mx-1" data-toggle="modal" data-target="#modal_verif{{$spjgu->id}}" title="verfikasi" disabled>
                          <i class="fas fa-file-invoice"></i>
                        </button>
                        @endif
						@if ($spjgu->Tag_v == 0)
                        <div class="btn-group">
                          ...
                          <div class="dropdown-menu" role="menu" style="">
                            <a class="dropdown-item" href="{{route('spjguedit', $spjgu->id)}}"><i class="fas fa-edit"></i> Edit</a>
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                          <div class="modal-content">
                            ...
                            <div class="modal-footer">
                              <form action="{{route('spjgu.verifikasi',$spjgu->id)}}" method="post">
                                @csrf
                      ...
                      <div class="modal fade" id="modal{{ $spjgu->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                          <div class="modal-content">
                            ...
                            <div class="modal-footer">
                              <form action="{{ route('spjgu.destroy', $spjgu->id) }}" method="post" class="">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    function Mymonth()
    {
      var slug = $('#bulan').val();
      console.log(slug);
      var url = '{{ route("spjgu.bulan", ":slug") }}';
