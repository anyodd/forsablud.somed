      ...
      <div class="col">
        <div class="card shadow-lg mt-2">
          ...
          <div class="card-body px-2 py-2">
            <a href="{{ route('setorpajak.create_setor',$id_rekan) }}" class="btn btn-sm btn-primary mb-2"><i class="fas fa-list-alt"></i> Setor Pajak
            </a>
            <a href="{{ route('setorpajak.index') }}" class="btn btn-sm btn-info mb-2"><i class="fas fa-angle-double-left"></i> Kembali
            </a>
              ...
              <table class="table table-sm table-bordered table-hover mb-0" id="example" width="100%" cellspacing="0">
                <thead class="thead-light">
                  <tr>
                    ...
                    <th class="text-center" style="vertical-align: middle;">Setor (Rp)</th>
                    <th class="text-center" style="vertical-align: middle; width: 10%;">Aksi</th>
                  </tr>
                </thead>
                <tbody>
                  @if (!empty($pajakrinci))
                  @foreach($pajakrinci as $number => $pajak)
                  <tr>
                    ...
                    <td>{{ $pajak->No_bill }}</td>                  
                    <td>{{ $pajak->No_ntpn }}</td>                 
                    <td class="text-right">{{ number_format($pajak->rupiah,2,',','.') }}</td>                 
                    <td>
                      <div class="row justify-content-center" >
                        <form action="{{ route('setorpajak.listdetail', $pajak->id_taxtor) }}" method="get" class="">
