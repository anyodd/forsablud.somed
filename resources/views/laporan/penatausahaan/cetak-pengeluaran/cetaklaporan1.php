...
class PDF extends fpdf
{
    ...
    function breakPageTableHeader($left, $w)
    {
        ...
        $this->Cell($w[2], 8, 'No SPP', 'LTRB', 0, 'C');
...
$pdf->SetXY(35, $pdf->getY() + 2);
$pdf->SetFont('Arial', 'B', 9);
$pdf->MultiCell(150, 4, 'REGISTER SURAT PERMINTAAN PEMBAYARAN (SPP)', '', 'C', 0);
$pdf->SetXY(35, $pdf->getY() + 2);
$pdf->SetFont('Arial', '', 9);
...
foreach ($model['data']['registerSpp'] as $data) {
    ...
    $pdf->MultiCell($w[1], 4, $lastDate != $data->spp_tgl ? date('d-m-Y', strtotime($data->spp_tgl)) : '', '', 'L');
