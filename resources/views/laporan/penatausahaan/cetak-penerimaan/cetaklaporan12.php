...
$xcurrent = $xcurrent + $w[0];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, ": ".$model['data']['ambilskpd'][0]->kode_skpd, '', 'L', 0);
$xcurrent = $xcurrent + $w[1];
$pdf->SetXY($xcurrent, $y);
...
$pdf->SetXY($left, $y);
$xcurrent = $left;
$pdf->MultiCell($w[0], 4, "UNIT", '', 'L', 0);
$xcurrent = $xcurrent + $w[0];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, ": ".$model['data']['ambilunit'][0]->kode_unit, '', 'L', 0);
$y = $pdf->GetY() + 1;
$xcurrent = $xcurrent + $w[1];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, "", '', 'L', 0);
...
$pdf->SetFont('Arial', 'B', 9);
$xcurrent = $left;
$pdf->MultiCell($w[0], 4, "PERIODE", '', 'L', 0);
$xcurrent = $xcurrent + $w[0];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, ": ".$pdf->bulan($bulan), '', 'L', 0);
$y = $pdf->GetY() + 1;
$xcurrent = $xcurrent + $w[1];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, "", '', 'L', 0);
...
foreach ($model['data']['spjfooter'] as $data) {
    $totalPenerimaan += $data->Terima;
}
$y = $pdf->GetY() + 4;
$pdf->SetXY($left, $y);
$pdf->SetFont('Arial', 'B', 9);
$xcurrent = $left;
$pdf->MultiCell($w[0], 4, 'A. ', '', 'L', 0);
$xcurrent = $xcurrent + $w[0]-20;
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, "Jumlah Penerimaan", '', 'L', 0);
$xcurrent = $xcurrent + $w[1];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[2], 4, "", '', 'L', 0);
$xcurrent = $xcurrent + $w[3];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[3], 4, number_format($totalPenerimaan, 2, ',', '.'), '', 'R', 0);
...
foreach ($model['data']['spjfooter'] as $data) {
    ...
    $pdf->SetFont('Arial', '', 9);
    $xcurrent = $left;
    $pdf->MultiCell($w[0], 6, '', '', 'L', 0);
    $xcurrent = $xcurrent + $w[0]-20;
    $pdf->SetXY($xcurrent, $y);
    $uraian = null;
    switch ($data->kode_summary_spj ) {
        case 1:
            $uraian = '1. Tunai melalui Bendahara Penerimaan';
            break;
        case 2:
            $uraian = '2. Tunai melalui Bendahara Penerimaan Pembantu';
            break;
        case 3:
            $uraian = '3. Melalui ke Rekening Bendahara Penerimaan';
            break;
        case 4:
            $uraian = '4. Melalui ke Rekening Kas Umum Daerah';
            break;
        default:
            # code...
            break;
    }
    $pdf->MultiCell($w[1], 6, $uraian, '', 'L', 0);
    $xcurrent = $xcurrent + $w[1];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[2], 6,  $data->kode_summary_spj !== 5 ? number_format($data->Terima, 2, ',', '.')  : '', '', 'R', 0);
    $totalPenerimaan += $data->Terima;
}
...
$pdf->SetFont('Arial', 'B', 9);
$xcurrent = $left;
$pdf->MultiCell($w[0], 4, 'B. ', '', 'L', 0);
$xcurrent = $xcurrent + $w[0]-20;
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, "Jumlah Penerimaan yang harus disetorkan(A1+A2+A3)", '', 'L', 0);
$xcurrent = $xcurrent + $w[1];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[2], 4, "", '', 'L', 0);
$xcurrent = $xcurrent + $w[3];
...
$pdf->SetFont('Arial', 'B', 9);
$xcurrent = $left;
$pdf->MultiCell($w[0], 4, 'C. ', '', 'L', 0);
$xcurrent = $xcurrent + $w[0]-20;
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, "Jumlah Penyetoran", '', 'L', 0);
$xcurrent = $xcurrent + $w[1];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[2], 4, "", '', 'L', 0);
$xcurrent = $xcurrent + $w[3];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[3], 4, number_format($totalPengeluaran, 2, ',', '.'), '', 'R', 0);

$y = $pdf->GetY() + 6;
$pdf->SetXY($left, $y);
$pdf->SetFont('Arial', 'B', 9);
$xcurrent = $left;
$pdf->MultiCell($w[0], 4, 'D. ', '', 'L', 0);
$xcurrent = $xcurrent + $w[0]-20;
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 4, "	Saldo Kas di Bendahara", '', 'L', 0);
$xcurrent = $xcurrent + $w[1];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[2], 4, "", '', 'L', 0);
$xcurrent = $xcurrent + $w[3];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[3], 4, number_format($totalPenerimaan-$totalPengeluaran, 2, ',', '.'), '', 'R', 0);

$totalSaldo = 0;
foreach ($model['data']['spjfooter1'] as $data) {
    $y = $pdf->GetY()+1;
    $pdf->SetFont('Arial', '', 9);
    $xcurrent = $left;
    $pdf->MultiCell($w[0], 6, '', '', 'L', 0);
    ...
    switch ($data->Saldo ) {
        case 1:
            $uraian = '1. Bendahara Penerimaan';
            break;
        case 2:
            $uraian = '2. Bendahara Penerimaan Pembantu';
            break;
        case 3:
            $uraian = '3. dst .....';
            break;
    ...
    $xcurrent = $xcurrent + $w[1];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[2], 6,  $data->Saldo == 1 ? number_format($totalPenerimaan-$totalPengeluaran, 2, ',', '.')  : number_format(0, 2, ',', '.'), '', 'R', 0);
}
