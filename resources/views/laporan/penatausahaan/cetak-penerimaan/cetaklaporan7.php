...
class PDF extends fpdf
{
    ...
    function breakPageTableHeader($left, $w)
    {
        ...
        $this->Cell($w[4], 8, 'Nilai', 'LTRB', 0, 'C');
        $this->ln();
    }
}
...
$pdf->SetXY(35, $pdf->getY() + 2);
$pdf->MultiCell(150, 4, 'REGISTER TANDA BUKTI PENERIMAAN', '', 'C', 0);
$pdf->SetXY(35, $pdf->getY() + 2);
$pdf->SetFont('Arial', '', 9);
...
foreach ($model['data']['registertbp'] as $data) {

    $yMaxAfter = max(
        $pdf->getY() + $pdf->GetMultiCellHeight($w[1], 5, $data->tgl_bp),
        $pdf->getY() + $pdf->GetMultiCellHeight($w[2], 5, $data->no_bp),
    ...
    $xcurrent = $xcurrent + $w[0];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[1], 4, $lastDate != $data->tgl_bp ? date('d-m-Y', strtotime($data->tgl_bp)) : '', '', 'L');
    $xcurrent = $xcurrent + $w[1];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[2], 4, $data->no_bp, '', 'L');
    ...
    $lastDate = $data->tgl_bp;
    $lastNoBukti = $data->no_bp;
    $total += $data->nilai;
}
