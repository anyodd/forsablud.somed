...
class PDF extends fpdf
{
    ...
    function breakPageTableHeader($left, $w)
    {
        ...
        $this->Cell($w[3] + $w[4] + $w[5], 6, 'PENETAPAN TAHUN LALU', 'LBTR', 0, 'C');
        $this->ln();
        $y = $this->GetY();
        $this->SetXY($left + $w[0] + $w[1] + $w[2], $y);
        $this->Cell($w[3] + $w[4] + $w[5], 6, 'PENERIMAAN TAHUN INI', 'LBTR', 0, 'C');
        $this->ln();
        $y = $this->GetY();
        $this->SetXY($left + $w[0] + $w[1] + $w[2], $y);
        $this->Cell($w[3] + $w[4] + $w[5], 6, 'PENYETORAN TAHUN INI', 'LBTR', 0, 'C');
        $this->ln();
    }
}
...
$pdf->SetXY(35, ($pdf->getY() + 2));
$pdf->SetFont('Arial', 'B', 9);
$pdf->MultiCell(290, 4, strtoupper("DAFTAR PEMBAYARAN PIUTANG TAHUN LALU"), '', 'C', 0);
$pdf->SetXY(35, ($pdf->getY() + 2));
$pdf->SetFont('Arial', 'B', 9);
