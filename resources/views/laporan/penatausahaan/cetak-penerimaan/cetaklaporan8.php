...
class PDF extends fpdf
{
    ...
    function breakPageTableHeader($left, $w)
    {
        ...
        $this->Cell($w[2], 8, 'No. STS', 'LTRB', 0, 'C');
        $this->Cell($w[3], 8, 'Uraian', 'LTRB', 0, 'C');
        $this->Cell($w[4], 8, 'Nilai STS', 'LTRB', 0, 'C');
        $this->ln();
    }
}
...
$pdf->SetXY(35, $pdf->getY() + 2);
$pdf->MultiCell(150, 4, 'REGISTER SURAT TANDA SETORAN (STS)', '', 'C', 0);
$pdf->SetXY(35, $pdf->getY() + 2);
$pdf->SetFont('Arial', '', 9);
...
$pdf->ln(1);
$w = [10, 16, 43, 96, 30]; // Tentukan width masing-masing kolom
...
foreach ($model['data']['registersts'] as $data) {

    $yMaxAfter = max(
        $pdf->getY() + $pdf->GetMultiCellHeight($w[1], 5, $data->tgl_sts),
        $pdf->getY() + $pdf->GetMultiCellHeight($w[2], 5, $data->no_sts),
        $pdf->getY() + $pdf->GetMultiCellHeight($w[3], 5, $data->uraian),
        $pdf->getY() + $pdf->GetMultiCellHeight($w[4], 5, number_format($data->nilai, 0, ',', '.')),
    );
    ...
    $xcurrent = $xcurrent + $w[0];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[1], 4, $lastDate != $data->tgl_sts ? date('d-m-Y', strtotime($data->tgl_sts)) : '', '', 'L');
    $xcurrent = $xcurrent + $w[1];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[2], 4, $data->no_sts, '', 'L');
    ...
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[4], 4, number_format($data->nilai, 2, ',', '.'), '', 'R');
    ...
    $lastDate = $data->tgl_sts;
    $lastNoBukti = $data->no_sts;
    $total += $data->nilai;
}
...
$pdf->Cell($w[3], 6, 'TOTAL', 'BR', 0, 'R');
$pdf->Cell($w[4], 6, number_format($total, 2, ',', '.'), 'BR', 0, 'R');
$pdf->ln();
