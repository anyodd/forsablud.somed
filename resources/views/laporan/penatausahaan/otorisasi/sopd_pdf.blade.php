<!DOCTYPE html>
<html>
@include('laporan.penatausahaan.otorisasi.pdf_head')
...
<body>
  ...
  <table class="table table-sm table-bordered" id="" width="100%" cellspacing="0">
    <tbody>
      <tr class="text-center">
        ...
        <td colspan="4">
          ...
          SURAT OTORISASI PENCAIRAN DANA (OPD) UP/GU/LS * <br>
          Nomor {{$data->No_oto}}
        </td>
      ...
      <tr>
        ...
        <td colspan="3">
          ...
          : {{ $qr_spm[0]->No_oto }} <br>
          : {{ Carbon\Carbon::parse($qr_spm[0]->Dt_oto)->isoFormat('DD MMMM Y') }} <br>
          : {{ nm_unit() }}
        </td>
        <td colspan="3">
          Dari : Direktur <br>
      ...
      <tr>
        ...
        <td colspan="6">
          : {{ $bank[0]->Ur_Bank ?? "---" }} <br>
          Hendaklah mencairkan/memindahbukukan dari Bank rekening nomor : {{ $bank[0]->No_Rek ?? "---" }} <br>
          @if (count($qr_pot) > 0 && in_array($qr_pot['0']->Ko_SPi,array('4','6')) == 0)
            : Rp {{ number_format($qr_spm->where('Kode', '1')->sum('Jumlah')-$qr_pot->sum('tax_Rp'),2,',','.') }}<br>
            ( {{ ucwords(terbilang($qr_spm->where('Kode', '1')->sum('Jumlah')-$qr_pot->sum('tax_Rp'),2,',','.')) }} Rupiah )
          @else
            : Rp {{ number_format($qr_spm->where('Kode', '1')->sum('Jumlah'),2,',','.') }}<br>
            ( {{ ucwords(terbilang($qr_spm->where('Kode', '1')->sum('Jumlah'),2,',','.')) }} Rupiah )
          @endif
      ...
      <tr>
        ...
        <td colspan="6">
          : {{ $qr_spm[0]->Trm_Nm }} <br>
          : {{ $qr_spm[0]->Trm_NPWP }} <br>
          : {{ $qr_spm[0]->Trm_rek }} <br>
          : {{ $qr_spm[0]->Trm_bank }} <br>
          : {{ $qr_spm[0]->Ur_oto }} <br>
        </td>
      </tr>
      ...
      @if (count($qr_spm->where('Kode', '1')) > 0)
      @foreach($qr_spm->where('Kode', '1') as $number => $dt)
      <tr class="text-center">
      ...
      <tr class="text-center font-weight-bold">
        ...
        <td style="width: 5%; border-right-style: hidden;">Rp</td>
        <td class="text-right" style="width: 20%">{{  number_format($qr_spm->where('Kode', '1')->sum('Jumlah'), 2, ",", ".") }}</td>
      </tr>
    ...
    <tr>
      <td colspan="8">Informasi: (Potongan pajak tidak mengurangi jumlah pembayaran OPD)</td>
    </tr>
    <thead>
      <tr class="text-center">
        <td>No</td>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td colspan="3" class="text-right">
          <br>
          {{  number_format($qr_spm->where('Kode', '1')->sum('Jumlah'), 2, ",", ".") }} <br>
          @if (count($qr_pot) > 0 && in_array($qr_pot['0']->Ko_SPi,array('4','6')) == 0)
          ...
          @endif
          @if (count($qr_pot) > 0 && in_array($qr_pot['0']->Ko_SPi,array('4','6')) == 0)
            {{  number_format($qr_spm->where('Kode', '1')->sum('Jumlah')-$qr_pot->sum('tax_Rp'), 2, ",", ".") }}
          @else
            {{  number_format($qr_spm->where('Kode', '1')->sum('Jumlah'), 2, ",", ".") }}
          @endif
      ...
      <tr style="border-top-style: hidden;">
        <td colspan="8">
          @if (count($qr_pot) > 0 && in_array($qr_pot['0']->Ko_SPi,array('4','6')) == 0)
            ( {{ ucwords(terbilang($qr_spm->where('Kode', '1')->sum('Jumlah')-$qr_pot->sum('tax_Rp'))) }} Rupiah )
          @else
            ( {{ ucwords(terbilang($qr_spm->where('Kode', '1')->sum('Jumlah'))) }} Rupiah )
          @endif
        </td>
      </tr>
      ...
      <tr>
        ...
        <td colspan="3" class="text-center">
          ...
          {{nm_ibukota()}}, {{ \Carbon\Carbon::parse($qr_spm[0]->Dt_oto)->isoFormat('DD MMMM Y') }}<br>
          Direktur<br><br><br>
          {{$data->Nm_Kuasa}} <br>
          NIP {{$data->NIP_Kuasa}}
        </td>
