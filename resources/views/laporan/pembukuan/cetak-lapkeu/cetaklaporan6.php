...
$pdf->SetXY(35, $pdf->getY());
$pdf->MultiCell(150, 4, 'NERACA', '', 'C', 0);
$pdf->SetXY(35, $pdf->getY());
$pdf->SetFont('Arial', '', 9);
...
foreach ($model['data']['neraca'] as $data) {
    ...
    if ($kdrek1 != $data->kdrek1) {
        ...
        $xcurrent = $x;
        $pdf->SetXY($x + 2, $y);
        $pdf->MultiCell($w[0] - 2, 4, $data->nmrek1, '', 'L');
        $xcurrent = $xcurrent + $w[0];
        $pdf->SetXY($xcurrent + 2, $y);
        $pdf->MultiCell($w[1] - 2, 4, '', '', 'L');
        $y1 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
        $xcurrent = $xcurrent + $w[1];
        $pdf->SetXY($xcurrent, $y);
        $pdf->MultiCell($w[2], 4, '', '', 'R'); // $pdf->accountingNumberFormat($data->anggaran_program)
        $y2 = $pdf->GetY();
        $xcurrent = $xcurrent + $w[2];
        $pdf->SetXY($xcurrent, $y);
    ...
    if ($kdrek2 != $data->kdrek1 . '.' . $data->kdrek2) {
        ...
        $xcurrent = $x;
        $pdf->SetXY($x + 4, $y);
        $pdf->MultiCell($w[0] - 4, 4, $data->nmrek2, '', 'L');
        $xcurrent = $xcurrent + $w[0];
        $pdf->SetXY($xcurrent + 2, $y);
        $pdf->MultiCell($w[1] - 2, 4, '', '', 'R');
        $y1 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
        $xcurrent = $xcurrent + $w[1];
        $pdf->SetXY($xcurrent + 2, $y);
        $pdf->MultiCell($w[2] - 2, 4, '', '', 'R'); // $pdf->accountingNumberFormat($data->anggaran_kegiatan)
        $y2 = $pdf->GetY();
        $xcurrent = $xcurrent + $w[2];
        $pdf->SetXY($xcurrent, $y);
    ...
    $xcurrent = $x;
    $pdf->SetXY($x + 6, $y);
    $pdf->MultiCell($w[0] - 6, 4, $data->nmrek3, '', 'L');
    $xcurrent = $xcurrent + $w[0];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[1], 4, $pdf->accountingNumberFormat($data->saldo_akhir), '', 'R');
    $y1 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
    $xcurrent = $xcurrent + $w[1];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[2], 4, $pdf->accountingNumberFormat($data->saldo_awal), '', 'R');
    $y2 = $pdf->GetY();
    $xcurrent = $xcurrent + $w[2];
    $pdf->SetXY($xcurrent, $y);
