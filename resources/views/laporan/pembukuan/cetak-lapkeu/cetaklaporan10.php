...
$pdf->SetXY(35, ($pdf->getY()));
$pdf->SetFont('Arial', 'B', 9);
$pdf->MultiCell(290, 4, strtoupper("LAPORAN REALISASI ANGGARAN PENDAPATAN DAN BELANJA DAERAH SAP"), '', 'C', 0);
$pdf->SetXY(35, ($pdf->getY()));
$pdf->SetFont('Arial', '', 9);
...
$anggaran_jumlahtransfer = 0;
$nilaiRealisasiperiodelalutransfer = 0;
$nilaiRealisasitransfer = 0;
...
foreach ($model['data']['rincianLRA'] as $data) {
    ...
    $tigadigitawal = substr($data->koderekening, 0, 3);
    if (($jumlahkode == 9) && ($tigadigitawal != '4.2')) {
        continue;
    }
    if (($pecahkoderekening[0] == 4) && ($jumlahkode == 1)) {
    ...
    if (($pecahkoderekening[0] == 5) && ($jumlahkode == 1)) {
    ...
    if (($pecahkoderekening[0] == 6) && ($jumlahkode == 1)) {
        $anggaran_jumlahtransfer = $data->temp_jumlah;
        if (isset($data->nilaiRealisasiperiodelalu)) {
            $nilaiRealisasiperiodelalutransfer = $data->nilaiRealisasiperiodelalu;
        }
        $nilaiRealisasitransfer = $data->nilaiRealisasi;
    }


    $pecahkoderekening_temp = explode(".", $koderekening_temp);

    if (($data->koderekening == '7')) {

        $surplusdefisit_ang = $anggaran_jumlah4 - $anggaran_jumlah5 - $anggaran_jumlahtransfer;
        $surplusdefisitperiodelalu_real = $nilaiRealisasiperiodelalu4 - $nilaiRealisasiperiodelalu5 - $nilaiRealisasiperiodelalutransfer;
        $surplusdefisit_real = $nilaiRealisasi4 - $nilaiRealisasi5 - $nilaiRealisasitransfer;
        ...
        $pdf->SetXY($x, $y);
        $xcurrent = $x;
        $pdf->MultiCell($w[0], 8, '', 'BT', 'L');
        $xcurrent = $xcurrent + $w[0];

        $pdf->SetXY($xcurrent, $y);

        $pdf->SetFont('Arial', 'B', 8);
        $pdf->MultiCell($w[1], 8, 'SURPLUS / DEFISIT', 'BT', 'R');
        ...
        $pdf->SetXY($xcurrent, $y);
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell($w[2], 8, $tampil_ang1, 'T', 'R');
        ...
        $xcurrent = $xcurrent + $w[2];
        $pdf->SetXY($xcurrent, $y);
        $pdf->MultiCell($w[3], 8, $tampil_real1, 'T', 'R');
        ...
        $pdf->SetXY($xcurrent, $y);
        //didapat dari tampil_real1/tampil_ang1
        //tampil_real1 adalah surplusdefisit_real
        //tampil_ang1 adalah surplusdefisit_ang
        if ($surplusdefisit_ang <= 0) {
            $pdf->MultiCell($w[4], 8, '0', 'T', 'R');
        } else {
            $persen = $surplusdefisit_real / $surplusdefisit_ang * 100;
            $pdf->MultiCell($w[4], 8, number_format($persen, 2, ',', '.'), 'T', 'R');
        }
        ///
        ...
        if ($tampil_periodelalureal1) {
            $pdf->MultiCell($w[5], 8, $tampil_periodelalureal1, 'T', 'R');
        } else {
            $pdf->MultiCell($w[5], 8, number_format(0, 2, ',', '.'), 'T', 'R');
        }
        ///
    ...
    $pdf->SetXY($x, $y);
    $xcurrent = $x;
    $pdf->MultiCell($w[0], 5, $data->koderekening, '', 'L');
    $xcurrent = $xcurrent + $w[0];
    ...
    if ($jumlahkode == 3) {
        ...
        $pdf->MultiCell($w[1] - 4, 5, $data->uraian, '', 'L');
    } else {
        if ($jumlahkode == 6) {
            $pdf->SetFont('Arial', '', 8);
            $pdf->SetXY($xcurrent + 6, $y);
            $pdf->MultiCell($w[1] - 6, 5, $data->uraian, '', 'L');
        } else {
            if ($jumlahkode == 9) {
                $pdf->SetXY($xcurrent + 8, $y);
                $pdf->MultiCell($w[1] - 8, 5, $data->uraian, '', 'L');
            } else {
                $pdf->SetFont('Arial', 'B', 8);
    ...
    $pdf->SetFont('Arial', '', 8);
    if ($jumlahkode <= 12) {
        $pdf->MultiCell($w[2], 5, number_format($data->temp_jumlah, 2, ',', '.'), 'T', 'R');
    } else {
        $pdf->MultiCell($w[2], 5, number_format($data->temp_jumlah, 2, ',', '.'), '', 'R');
    }
    $y2 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
    ...
    if ($jumlahkode <= 12) {
        $pdf->MultiCell($w[3], 5, number_format($data->nilaiRealisasi, 2, ',', '.'), 'T', 'R');
    } else {
        $pdf->MultiCell($w[3], 5, number_format($data->nilaiRealisasi, 2, ',', '.'), '', 'R');
    }
    $y3 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
    ///
    ...
    if (isset($data->nilaiRealisasiperiodelalu)) {
        if ($jumlahkode <= 12) {
            $pdf->MultiCell($w[5], 5, number_format($data->nilaiRealisasiperiodelalu, 2, ',', '.'), 'T', 'R');
        } else {
            $pdf->MultiCell($w[5], 5, number_format($data->nilaiRealisasiperiodelalu, 2, ',', '.'), '', 'R');
        }
    } else {
        if ($jumlahkode <= 12) {
            $pdf->MultiCell($w[5], 5, number_format(0, 2, ',', '.'), 'T', 'R');
        } else {
            $pdf->MultiCell($w[5], 5, number_format(0, 2, ',', '.'), '', 'R');
        }
    }
    ///
...
if ($pecahkoderekening[0] == 5) {
    ...
    $pdf->SetXY($x, $y);
    $xcurrent = $x;
    $pdf->MultiCell($w[0], 8, '', 'T', 'L');
    $xcurrent = $xcurrent + $w[0];

    $pdf->SetXY($xcurrent, $y);

    $pdf->SetFont('Arial', 'B', 8);
    $pdf->MultiCell($w[1], 8, 'SURPLUS / DEFISIT', 'T', 'R');
    $y1 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
    $xcurrent = $xcurrent + $w[1];
    $pdf->SetXY($xcurrent, $y);
...
if ($pecahkoderekening[0] == 7) {
    ...
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->MultiCell($w[1], 8, 'PEMBIAYAAN NETTO', 'T', 'R');
    $y1 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
    $xcurrent = $xcurrent + $w[1];
    $pdf->SetXY($xcurrent, $y);

    $pdf->SetFont('Arial', '', 8);
    $pdf->MultiCell($w[2], 8, $tampil_PEMBIAYAAN_ANG1, 'T', 'R');
    $y2 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan

    ///
    $xcurrent = $xcurrent + $w[2];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[3], 8, $tampil_PEMBIAYAAN_REAL1, 'T', 'R');
    $y3 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
    ///
    ...
    if ($tampil_PEMBIAYAAN_periodelaluREAL1) {
        $pdf->MultiCell($w[5], 8, $tampil_PEMBIAYAAN_periodelaluREAL1, 'T', 'R');
    } else {
        $pdf->MultiCell($w[5], 8, number_format(0, 2, ',', '.'), 'T', 'R');
    }
    ///
...
$pdf->SetXY($x, $y);
$xcurrent = $x;
$pdf->MultiCell($w[0], 8, '', 'LB', 'C');
$xcurrent = $xcurrent + $w[0];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[1], 8, 'SISA LEBIH / KURANG PEMBIAYAAN TAHUN BERKENAAN', 'BR', 'C');
$y1 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
$xcurrent = $xcurrent + $w[1];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[2], 8, $tampil_TOTAL_ANG1, 'BR', 'R');
$y2 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan

///
$xcurrent = $xcurrent + $w[2];
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[3], 8, $tampil_TOTAL_REAL1, 'BR', 'R');
$y3 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
///
...
$xcurrent = $xcurrent + $w[3];
$pdf->SetXY($xcurrent, $y);
//didapat dari tampil_total_real1/tampil_total_ang1
//tampil_total_real1 adalah total_real
//tampil_total_ang1 adalah total_ang
if ($TOTAL_ANG <= 0) {
    $pdf->MultiCell($w[4], 8, '0', 'BR', 'R');
} else {
    $persen = $TOTAL_REAL / $TOTAL_ANG * 100;
    $pdf->MultiCell($w[4], 8, number_format($persen, 2, ',', '.'), 'BR', 'R');
}
///
...
if ($tampil_TOTAL_periodelaluREAL1) {
    $pdf->MultiCell($w[5], 8, $tampil_TOTAL_periodelaluREAL1, 'BR', 'R');
} else {
    $pdf->MultiCell($w[5], 8, number_format(0, 2, ',', '.'), 'BR', 'R');
}
///
