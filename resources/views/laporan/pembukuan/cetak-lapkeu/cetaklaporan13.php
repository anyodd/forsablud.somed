...
$pdf->SetXY(35, ($pdf->getY()));
$pdf->SetFont('Arial', 'B', 9);
$pdf->MultiCell(290, 4, strtoupper("LAPORAN OPERASIONAL SAP"), '', 'C', 0);
$pdf->SetXY(35, ($pdf->getY()));
$pdf->SetFont('Arial', '', 9);
...
$w = [20, 160, 35, 35, 35, 15]; // Tentukan width masing-masing kolom 300
...
$pdf->MultiCell($w[1] - 2, 5, 'KEGIATAN OPERASIONAL', '', 'L');
$tb = '';
...
$pdf->SetXY($xcurrent, $y);
$pdf->MultiCell($w[4], 5, '', '', 'R');

$xcurrent = $xcurrent + $w[4];
$pdf->SetXY($xcurrent, $y);

$pdf->MultiCell($w[5], 5, '', '', 'R');
...
foreach ($model['data']['rincianLO'] as $data) {
    ...
    if ($jumlahkode == 1) {
        ...
        if ($jumlahkode == 4) {
            ...
            $pdf->MultiCell($w[1] - 5, 5, $data->uraian, '', 'L');
            $tb = '';
        } else {
            $pdf->SetFont('Arial', '', 8);
            $pdf->SetXY($xcurrent + 7, $y);
            $pdf->MultiCell($w[1] - 7, 5, $data->uraian, '', 'L');
            $tb = '';
        }
    }
    ...
    $pdf->SetFont('Arial', '', 8);
    $pdf->MultiCell($w[2], 5, number_format($data->saldo_tahun_ini, 2, ',', '.'), $tb, 'R');

    $y2 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
    $xcurrent = $xcurrent + $w[2];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[3], 5, number_format($data->saldo_tahun_lalu, 2, ',', '.'), $tb, 'R');
    ...
    if ($data->kenaikan_penurunan < 0) {
        ...
        $pdf->MultiCell($w[4], 5, '(' . number_format($varbuatan_kenaikan_penurunan, 2, ',', '.') . ')', $tb, 'R');
    } else {
        $pdf->MultiCell($w[4], 5, number_format($data->kenaikan_penurunan, 2, ',', '.'), $tb, 'R');
    }
    ...
    if ($data->saldo_tahun_lalu <= 0 && $data->kenaikan_penurunan < 0) {
        $pdf->MultiCell($w[5], 5, '0', $tb, 'R');
    } else { 
		if ($data->saldo_tahun_lalu <= 0 ) {
			$pdf->MultiCell($w[5], 5, '0', $tb, 'R');
		} else { 
			if (!empty($data->saldo_tahun_lalu)) {
				$pdf->MultiCell($w[5], 5, '0', $tb, 'R');
			} else { 
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($xcurrent + 2, $y);
$pdf->MultiCell($w[1] - 2, 5, 'KEGIATAN NON OPERASIONAL', '', 'L');
$tb = '';
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($xcurrent + 2, $y);
$pdf->MultiCell($w[1] - 2, 5, 'POS LUAR BIASA', '', 'L');
$tb = '';
