      ...
      <div class="col">
        <div class="card shadow-lg mt-2">
          <div class="card-header bg-info py-2">
            <h5 class="card-title font-weight-bold">Rincian LS Tagihan</h5> 
          </div>
          <div class="card-body">
            <div class="card">
                ...
                <div class="row">
                  <div class="col-sm-12">
                    <div class="form-group row">
                      ...
                      <div class="col-sm-2">
                        <input type="text" class="form-control" name="Dt_SPi" id="Dt_SPi" value="{{date_format($spi->Dt_SPi,'d-m-Y')}}" disabled>
						<input type="text" class="form-control" name="Old_Dt_SPi" id="Old_Dt_SPi" value="{{$spi->Dt_SPi}}" hidden>
                      </div>
                    </div>
                  </div>
                  <div class="col-sm-12">
            ...
            <div class="row justify-content-center mb-2">
              ...
              <a href="{{ route('lsnonkontrak.bulan',Session::get('bulan')) }}" class="btn btn-info"><i class="fa fa-reply"></i> Kembali</a>
            </div>
...
<script>
  ...
  let route = '{{ URL::route('lsnonkontrak.rincian',':id') }}';
  ...
  $('#btn_tambahrinci').click(function (e) { 
    ...
    //let id    = {{tahun()}};
	let id      = document.getElementById('Old_Dt_SPi').value;
    let route   = '{{ URL::route('lsnonkontrak.list_tagihan',':id') }}';
    let url     = route.replace(':id',id);
    var tablelist =
    $('.tablelist').DataTable({
      pageLength  : 25,
  ...
  $('body').on('click','.delete', function () {
    ...
    let route = '{{ URL::route('lsnonkontrak.hapusrinciantagihan',':id') }}';
    let url   = route.replace(':id',id);
  ...
  $(document).on('click','#btn_hapusrinci', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          type : 'POST',
          url  : '{{route('lsnonkontrak.hapusrincian')}}',
  ...
  $(document).on('click','#submit', function () {
    ...
    $.ajax({
      type : 'POST',
      url  : '{{route('lsnonkontrak.tambahrincian')}}',
