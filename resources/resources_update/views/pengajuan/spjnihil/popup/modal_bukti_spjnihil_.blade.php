<!-- Modal Bukti Rincian -->
<div class="modal fade" id="modalBuktiSpjnihil" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-bold" id="exampleModalLabel">BUKTI YANG DIAJUKAN - SPP NIHIL</h5>
            </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <table class="table table-sm table-bordered table-hover mb-0" id="tabelTap" width="100%" cellspacing="0">
                                            ...
                                            <tbody>
                                            <?php $no = 0;?>
                                            @foreach($buktispjnihil as $buktispjnihil)
                                            <?php $no++ ;?>
                                            <tr>
                                                <td style="text-align: center">{{$no}}</td>                
                                                <td>{{ $buktispjnihil->no_bp }}</td>                      
                                                <td>{{ $buktispjnihil->dt_bp }}</td>                      
                                                <td>{{ $buktispjnihil->ur_bprc }}</td>                      
                                                <td>{{ $buktispjnihil->nm_bucontr }}</td>  
                                                <td>{{ number_format($buktispjnihil->to_rp,2,',','.') }}</td>  
                                                <td>
                                                    <button class="btn btn-primary py-0" data-dismiss="modal" title="Pilih data" id="select"
                                                        data-val1 = "{{ $buktispjnihil->no_bp }}"
                                                        data-val2 = "{{ $buktispjnihil->ko_bprc }}"
                                                        data-val3 = "{{ $buktispjnihil->ko_skeg1 }}"
                                                        data-val4 = "{{ $buktispjnihil->ko_skeg2 }}"
                                                        data-val5 = "{{ $buktispjnihil->ko_rkk }}"
                                                        data-val6 = "{{ $buktispjnihil->ko_pdp }}"
                                                        data-val7 = "{{ $buktispjnihil->ko_pmed }}"
                                                        data-val8 = "{{ $buktispjnihil->rftr_bprc }}"
                                                        data-val9 = "{{ $buktispjnihil->dt_rftrbprc }}"
                                                        data-val10 = "{{ $buktispjnihil->ur_bprc }}"
                                                        data-val11 = "{{ $buktispjnihil->to_rp }}"
