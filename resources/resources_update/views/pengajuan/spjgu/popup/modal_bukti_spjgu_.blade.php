<!-- Modal Bukti Rincian -->
<div class="modal fade" id="modalBuktiSpjgu" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-bold" id="exampleModalLabel">BUKTI YANG DIAJUKAN - SPP GANTI UANG (GU)</h5>
            </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <table class="table table-sm table-bordered table-hover mb-0" id="tabelTap" width="100%" cellspacing="0">
                                            ...
                                            <tbody>
                                            <?php $no = 0;?>
                                            @foreach($buktispjgu as $buktispjgu)
                                            <?php $no++ ;?>
                                            <tr>
                                                <td style="text-align: center">{{$no}}</td>                
                                                <td>{{ $buktispjgu->no_bp }}</td>                      
                                                <td>{{ $buktispjgu->dt_bp }}</td>                      
                                                <td>{{ $buktispjgu->ur_bprc }}</td>                      
                                                <td>{{ $buktispjgu->nm_bucontr }}</td>  
                                                <td>{{ number_format($buktispjgu->to_rp,2,',','.') }}</td>                      
                                                <td>
                                                    <button class="btn btn-primary py-0" data-dismiss="modal" title="Pilih data" id="select"
                                                        data-val1 = "{{ $buktispjgu->no_bp }}"
                                                        data-val2 = "{{ $buktispjgu->ko_bprc }}"
                                                        data-val3 = "{{ $buktispjgu->ko_skeg1 }}"
                                                        data-val4 = "{{ $buktispjgu->ko_skeg2 }}"
                                                        data-val5 = "{{ $buktispjgu->ko_rkk }}"
                                                        data-val6 = "{{ $buktispjgu->ko_pdp }}"
                                                        data-val7 = "{{ $buktispjgu->ko_pmed }}"
                                                        data-val8 = "{{ $buktispjgu->rftr_bprc }}"
                                                        data-val9 = "{{ $buktispjgu->dt_rftrbprc }}"
                                                        data-val10 = "{{ $buktispjgu->ur_bprc }}"
                                                        data-val11 = "{{ $buktispjgu->to_rp }}"
