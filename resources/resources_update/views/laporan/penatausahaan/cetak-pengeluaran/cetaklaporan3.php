...
class PDF extends fpdf
{
    ...
    function breakPageTableHeader($left, $w)
    {
        ...
        $this->Cell($w[2], 8, 'No S-PD', 'LTRB', 0, 'C');
...
$pdf->SetXY(35, $pdf->getY() + 2);
$pdf->SetFont('Arial', 'B', 9);
$pdf->MultiCell(150, 4, 'REGISTER SURAT PENCAIRAN DANA (S-PD)', '', 'C', 0);
$pdf->SetXY(35, $pdf->getY() + 2);
$pdf->SetFont('Arial', '', 9);
...
foreach ($model['data']['registerSp2d'] as $data) {
    ...
    $yMaxAfter = max(
        $pdf->getY() + $pdf->GetMultiCellHeight($w[1], 5, $data->sp2d_tgl),
        $pdf->getY() + $pdf->GetMultiCellHeight($w[2], 5, $data->sp2d_no),
    ...
    $pdf->MultiCell($w[1], 4, $lastDate != $data->sp2d_tgl ? date('d-m-Y', strtotime($data->sp2d_tgl)) : '', '', 'L');
    $xcurrent = $xcurrent + $w[1];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[2], 4, $data->sp2d_no, '', 'L');
    ...
    $lastDate = $data->sp2d_tgl;
    $lastNoBukti = $data->sp2d_no;
    $total += $data->nilai;
}
