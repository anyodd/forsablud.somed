...
class PDF extends fpdf
{
    ...
    function breakPageTableHeader($left, $w)
    {
        ...
        $this->Cell($w[3], 8, 'Kode Rek Lawan', 'LBTR', 0, 'C');
        $this->Cell($w[4], 8, 'Uraian', 'LBTR', 0, 'C');
        $this->Cell($w[5], 8, 'Debet', 'LBTR', 0, 'C');
        $this->Cell($w[6], 8, 'Kredit', 'LBTR', 0, 'C');
        $this->Cell($w[7], 8, 'Saldo', 'LBTR', 0, 'C');
        $this->ln();
    }
}
...
foreach ($model['data']['daftarBuktiBukuBesar'] as $data) {
    if ($rekening != $data->kdrek1 . '.' . $data->kdrek2 . '.' . $data->kdrek3 . '.' . $data->kdrek4 . '.' . $data->kdrek5 . '.' . $data->kdrek6) {
        ...
        if ($rekening !== null) {
            ...
            $pdf->Cell($w[4], 6, 'JUMLAH', 'BL', 0, 'R');
            $pdf->Cell($w[5], 6, $pdf->accountingNumberFormat($totalDebet, 2, ',', '.'), 'BL', 0, 'R');
            $pdf->Cell($w[6], 6, $pdf->accountingNumberFormat($totalKredit, 2, ',', '.'), 'BL', 0, 'R');
            $pdf->Cell($w[7], 6, $pdf->accountingNumberFormat(($previousSaldoNormal == 'D' ? $saldo : -$saldo), 2, ',', '.'), 1, 0, 'R');
        ...
        $pdf->SetXY(35, ($pdf->getY()));
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->MultiCell(290, 4, strtoupper("Buku Besar Pembantu"), '', 'C', 0);
        $pdf->SetXY(35, ($pdf->getY()));
        $pdf->SetFont('Arial', '', 9);
        ...
        $w = [15, 20, 45, 35, 95, 30, 30, 30]; // Tentukan width masing-masing kolom
    ...
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[5], 5, ($data->kode == 1 ? $pdf->accountingNumberFormat($data->debet, 2, ',', '.') : ''), '', 'R');
    $xcurrent = $xcurrent + $w[5];
    $pdf->SetXY($xcurrent, $y);
    $pdf->MultiCell($w[6], 5, ($data->kode == 1 ? $pdf->accountingNumberFormat($data->kredit, 2, ',', '.') : ''), '', 'R');
    $xcurrent = $xcurrent + $w[6];
    $pdf->SetXY($xcurrent, $y);
    $saldo = $saldo + $data->debet - $data->kredit;
    $pdf->MultiCell($w[7], 5, $pdf->accountingNumberFormat(($data->saldo_normal == 'D' ? $saldo : -$saldo), 2, ',', '.'), '', 'R');
