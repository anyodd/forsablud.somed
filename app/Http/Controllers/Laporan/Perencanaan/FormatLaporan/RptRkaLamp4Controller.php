...
class RptRkaLamp4Controller extends Controller
{
	...
	public static function Laporan($tahun, $idkegiatan, $rkaperubahan = null)
    {
		...
		if($rkaperubahan==null){
			...
			$keg = DB::SELECT("SELECT DISTINCT m.Ur_Prg AS nmprogram, m.Ko_Prg AS kode_program, l.Ur_Keg AS nmkegiatan, l.Ko_Prg AS idprogram, CONCAT(LPAD(l.Ko_Prg,2,0),'.',l.Ko_Keg) AS kode_kegiatan
						...
						WHERE LEFT(j.Ko_unit1,18) = '".$row0->kode_unit."' AND CONCAT(LPAD(k.Ko_Prg,2,0),'.',k.Ko_Keg)='" . $idkegiatan."' " )[0];
			...
			$pagu = DB::SELECT("SELECT dat.Ko_urus AS Ko_urus, dat.kode_bid AS idbidang, dat.idunit, LEFT(dat.kode_subkegiatan,2) AS idprogram, LEFT(dat.kode_subkegiatan,7) AS idkegiatan,
				...
				FROM (
						...
						pf_bid n ON m.id_bidang=n.id_bidang INNER JOIN
						pf_urus o ON n.id_urus=o.id_urus
						WHERE a.Ko_Period = ".$tahun." AND LEFT(a.Ko_unit1,18) = '".$row0->kode_unit."' AND LEFT(a.ko_rkk,2) IN (5) AND a.To_Rp<>0 AND CONCAT(LPAD(k.Ko_Prg,2,0),'.',k.Ko_Keg)='" . $idkegiatan."'
					) AS dat
			...
			if(!empty($subkeg)) {
				...
				$query = DB::select("WITH rincbelanja_arsip AS ( 
					...
					FROM (
						...
						pf_bid n ON m.id_bidang=n.id_bidang INNER JOIN
						pf_urus o ON n.id_urus=o.id_urus
						WHERE a.Ko_Period = ".$tahun." AND LEFT(a.Ko_unit1,18) = '".$row0->kode_unit."' AND LEFT(a.ko_rkk,2) IN (5) AND a.To_Rp<>0 AND CONCAT(LPAD(k.Ko_Prg,2,0),'.',k.Ko_Keg,'.',LPAD(k.Ko_sKeg,3,0))='".$sk->kode_subkegiatan."' 
					) A
