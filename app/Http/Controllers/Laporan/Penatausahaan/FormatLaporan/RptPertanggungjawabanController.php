...
class RptPertanggungjawabanController extends Controller
{
    ...
    public static function Laporan($tahun, $bulan, $jn_spj)
    {
		...
		$rincianSpj = DB::select("SELECT A.kdrek1, A.kdrek2, A.kdrek3, A.kdrek4, A.kdrek5, A.kdrek6, rek.Ur_Rk6 AS nmrek6, A.Ko_Rkk, SUM(A.Anggaran) AS Anggaran, SUM(A.Terima_Lalu) AS Terima_Lalu, SUM(A.Setor_Lalu) AS Setor_Lalu, SUM(A.Terima_Lalu) - SUM(A.Setor_Lalu) AS Sisa_Lalu, SUM(A.Terima_Ini) AS Terima_Ini,
							...
							FROM (
									...
									WHERE  (A.Ko_Period = ".$tahun.") AND ( MONTH(CAST(D.dt_byr AS DATE)) <  ".$bulan." ) 
									...
									WHERE  (A.Ko_Period = ".$tahun.")  AND (MONTH(CAST(B.dt_sts AS DATE)) <  ".$bulan." ) AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) 	
									...
									WHERE  (A.Ko_Period = ".$tahun.")  AND ( MONTH(CAST(B.dt_byr AS DATE)) <  ".$bulan." ) AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) AND ( D.Ko_bp IN (1, 42, 43) )
									...
									WHERE  (A.Ko_Period = ".$tahun.")  AND ( MONTH(CAST(B.dt_byr AS DATE)) <  ".$bulan." ) AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) AND ( D.Ko_bp IN (11) )
									...
									FROM tb_sts B 
									INNER JOIN tb_stsrc A ON 	B.id_sts = A.id_sts
									WHERE  (A.Ko_Period = ".$tahun.")  AND ( MONTH(CAST(B.dt_sts AS DATE)) = ".$bulan." ) 
									AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) 	
									GROUP BY B.Ko_Period, B.Ko_unitstr, A.No_byr
									) A INNER JOIN (
									SELECT B.Ko_Period AS tahun, A.Ko_Rkk, B.Ko_unitstr, B.No_byr, SUM(B.real_rp) AS real_rp
									...
									INNER JOIN tb_bprc A ON B.id_bprc = A.id_bprc
									INNER JOIN tb_bp D ON A.id_bp = D.id_bp 
									WHERE  (A.Ko_Period = ".$tahun.")  AND ( MONTH(CAST(B.dt_byr AS DATE)) <= ".$bulan." ) AND ( D.Ko_bp IN (1, 42, 43) )
									AND (B.Ko_unitstr = LEFT('".kd_unit()."',18))	
									GROUP BY B.Ko_Period, B.Ko_unitstr, B.No_byr, A.Ko_Rkk
									) B ON A.tahun = B.tahun AND A.Ko_unitstr = B.Ko_unitstr AND A.No_byr = B.No_byr
									GROUP BY B.tahun, B.Ko_Rkk
									...
									INNER JOIN tb_bp D ON A.id_bp = D.id_bp 
									INNER JOIN pf_mapjr C ON A.Ko_Rkk=C.Lo_D
									WHERE  (A.Ko_Period = ".$tahun.")  AND ( MONTH(CAST(B.dt_byr AS DATE)) <= ".$bulan." ) AND ( D.Ko_bp IN (11) )
									AND (B.Ko_unitstr = LEFT('".kd_unit()."',18))	
									GROUP BY B.Ko_Period, B.Ko_unitstr, B.No_byr, C.Ko_Rkk
									) B ON A.tahun = B.tahun AND A.Ko_unitstr = B.Ko_unitstr AND A.No_byr = B.No_byr
									GROUP BY B.tahun, B.Ko_Rkk
							...
							GROUP BY A.kdrek1, A.kdrek2, A.kdrek3, A.kdrek4, A.kdrek5, A.kdrek6, rek.Ur_Rk6, A.Ko_Rkk
							ORDER BY A.Ko_Rkk ASC");
								
		$spjfooter = DB::select("SELECT 1 AS kode_summary_spj, 1 AS Saldo, A.Tunai AS kode, 'Penerimaan Tunai' AS Uraian, SUM(A.Terima) AS Terima, 0 AS Keluar
								FROM
									(		
									SELECT 2 AS Tunai, A.Ko_Period, A.Ko_unit1, D.dt_byr AS Tgl_Bukti, D.No_byr AS No_Bukti, 
									A.No_bp, B.dt_bp, B.Ur_bp, A.Ko_bprc,
									A.Ur_bprc, A.Ko_sKeg1, A.Ko_sKeg2, A.Ko_Rkk, 
									A.Ko_Pdp, A.ko_kas, A.To_Rp AS Terima, 0 AS Keluar			 
									FROM tb_bprc A 
									INNER JOIN  tb_bp B ON A.id_bp = B.id_bp 
									INNER JOIN tb_byr D ON A.id_bprc = D.id_bprc
									WHERE (A.Ko_Period = ".$tahun.") AND B.Ko_bp = 1 AND A.ko_kas = 2 AND (LEFT(A.Ko_unit1,18) = LEFT('".kd_unit()."',18)) AND ( MONTH(CAST(D.dt_byr AS DATE)) = ".$bulan." ) 
									) A
								UNION ALL
								SELECT 2 AS kode_summary_spj, 2 AS Saldo, 2 AS kode, 'Penerimaan Tunai Bpp' AS Uraian, 0 AS Terima, 0 AS Keluar
								UNION ALL
								SELECT 3 AS kode_summary_spj, 1 AS Saldo, A.Bank AS kode, 'Penerimaan Bank' AS Uraian, SUM(A.Terima) AS Terima, 0 AS Keluar
								FROM
									(
									SELECT 1 AS Bank, A.Ko_Period, A.Ko_unit1, D.dt_byr AS Tgl_Bukti, D.No_byr AS No_Bukti, 
									A.No_bp, B.dt_bp, B.Ur_bp, A.Ko_bprc,
									A.Ur_bprc, A.Ko_sKeg1, A.Ko_sKeg2, A.Ko_Rkk, 
									A.Ko_Pdp, A.ko_kas, A.To_Rp AS Terima, 0 AS Keluar			 
									FROM tb_bprc A 
									INNER JOIN  tb_bp B ON A.id_bp = B.id_bp 
									INNER JOIN tb_byr D ON A.id_bprc = D.id_bprc	
									WHERE (A.Ko_Period = ".$tahun.") AND B.Ko_bp = 1 AND A.ko_kas = 1 
									AND (LEFT(A.Ko_unit1,18) = LEFT('".kd_unit()."',18)) AND ( MONTH(CAST(D.dt_byr AS DATE)) = ".$bulan." ) 
									) A
								UNION ALL
								SELECT 4 AS kode_summary_spj, 3 AS Saldo, 2 AS kode, 'Penerimaan Bank Rkud' AS Uraian, 0 AS Terima, 0 AS Keluar
								UNION ALL
								SELECT 5 AS kode_summary_spj, 4 AS Saldo, 3 AS kode, 'Penyetoran' AS Uraian, 0 AS Terima, SUM(B.Keluar) AS Keluar
								FROM  
								(
									SELECT B.Ko_Period AS tahun, B.Ko_unitstr, A.No_byr
									FROM tb_sts B 
									INNER JOIN tb_stsrc A ON 	B.id_sts = A.id_sts
									WHERE  (B.Ko_Period = ".$tahun.") AND ( MONTH(CAST(B.dt_Sts AS DATE)) = ".$bulan." ) AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) 
									GROUP BY B.Ko_Period, B.Ko_unitstr, A.No_byr
										) A INNER JOIN 
								(
									SELECT B.Ko_Period AS tahun, B.Ko_unitstr, B.No_byr, SUM(B.real_rp) AS Keluar
									FROM tb_byr B 
									INNER JOIN tb_bprc A ON B.id_bprc = A.id_bprc 
									INNER JOIN tb_bp D ON A.id_bp = D.id_bp 
									LEFT JOIN pf_jnpdp F ON A.Ko_Pdp = F.Ko_Pdp		
									WHERE  (B.Ko_Period = ".$tahun.") AND ( MONTH(CAST(B.dt_byr AS DATE)) = ".$bulan." ) AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) AND (D.Ko_bp IN (1)) 
									GROUP BY B.Ko_Period, B.Ko_unitstr, B.No_byr
										) B ON A.tahun = B.tahun AND A.Ko_unitstr = B.Ko_unitstr AND A.No_byr = B.No_byr");
		$spjfooter1 = DB::select("SELECT 1 AS Saldo, 0 AS Terima, 0 AS Keluar
								UNION ALL
								SELECT 2 AS Saldo, 0 AS Terima, 0 AS Keluar
								UNION ALL
								SELECT 3 AS Saldo, 0 AS Terima, 0 AS Keluar");


		$data = [
			'rincianSpj' => $rincianSpj,
			'spjfooter' => $spjfooter,
			'spjfooter1' => $spjfooter1,
			'ambilbidang' => $bid,
			'ambilskpd' => $skpd,
			'ambilunit' => $unit,
			'refPemda' => $pemda,
