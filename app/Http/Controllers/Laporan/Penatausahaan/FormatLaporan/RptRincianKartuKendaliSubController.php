...
class RptRincianKartuKendaliSubController extends Controller
{
    ...
    public static function Laporan($tahun, $tgl_1, $tgl_2, $idprogram, $idkegiatan, $idsubkegiatan)
    {
		$data = null;
		$ambilbidang = 0;
		$ambilskpd = 0;
		...
		$Ko_tap = DB::SELECT("SELECT COALESCE(MAX(Ko_tap),".$Ko_tap_awl[0]->Ko_Tap.") AS Ko_tap FROM tb_tap WHERE Ko_Period = ".$tahun." AND LEFT(Ko_unit1,18) = '".kd_unit()."' AND CAST(Dt_Tap AS DATE) <= '".$tgl_2."' ");
		...
		if($apbd[0]->apbd == 0){
			$rincianSubkegiatan = DB::select("WITH anggaran AS (								
									...
									gabung AS (
										SELECT a.* FROM (
											SELECT 1 AS Kode,
												A.Ko_Period AS tahun, A.Ko_sKeg1, A.Ko_sKeg2, A.no_bukti, A.tgl_bukti, A.uraian, 
											...
											FROM
											(
												SELECT A.Ko_Period, A.dt_rftrbprc AS tgl_bukti, A.No_bp AS no_bukti, A.Ur_bprc AS uraian, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, SUM(A.spirc_Rp) AS nilai
												...
												WHERE ( CAST(D.Dt_npd AS DATE) <= '".$tgl_2."' ) AND (A.Ko_Period = ".$tahun.") AND (A.Ko_unitstr = LEFT('".kd_unit()."',18))
												AND B.Ko_SPi IN(4,6,8)
												GROUP BY A.Ko_Period, A.No_bp, D.Dt_npd, A.Ur_bprc, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, A.dt_rftrbprc
											) A
											GROUP BY A.Ko_Period, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, A.no_bukti, A.tgl_bukti, A.uraian
											UNION ALL
											SELECT 2 AS Kode,
												A.Ko_Period AS tahun, A.Ko_sKeg1, A.Ko_sKeg2, A.no_bukti, A.tgl_bukti, A.uraian, 
												LEFT(A.ko_rkk,2) AS kdrek1, SUBSTRING(A.ko_rkk,4,2) AS kdrek2, 
											...
											SELECT A.Ko_Period, A.dt_rftrbprc AS tgl_bukti, A.No_bp AS no_bukti, A.Ur_bprc AS uraian, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, SUM(A.spirc_Rp) AS nilai
													...
													GROUP BY A.Ko_Period, A.No_bp, D.Dt_npd, A.Ur_bprc, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, A.dt_rftrbprc
											UNION ALL
											SELECT A.Ko_Period, A.dt_rftrbprc AS tgl_bukti, A.No_bp AS no_bukti, A.Ur_bprc AS uraian, A.Ko_sKeg1, A.Ko_sKeg2, map.Ko_RKK, SUM(A.spirc_Rp) AS nilai
													FROM tb_spirc A INNER JOIN
													tb_spi B ON A.id_spi = B.id INNER JOIN
													tb_oto C ON B.id = C.id_spi INNER JOIN
													tb_npd D ON C.id = D.id_oto INNER JOIN
													...
													WHERE ( CAST(D.Dt_npd AS DATE) <= '".$tgl_2."' ) AND (A.Ko_Period = ".$tahun.") AND (A.Ko_unitstr = LEFT('".kd_unit()."',18))
													AND B.Ko_SPi IN(9)
													GROUP BY A.Ko_Period, A.No_bp, D.Dt_npd, A.Ur_bprc, A.Ko_sKeg1, A.Ko_sKeg2, map.Ko_RKK, A.dt_rftrbprc
											) A
											GROUP BY A.Ko_Period, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, A.no_bukti, A.tgl_bukti, A.uraian
											UNION ALL
											SELECT 1 AS Kode,
												A.tahun, A.Ko_sKeg1, A.Ko_sKeg2, A.no_bukti, A.tgl_bukti, A.uraian, 
												A.kdrek1, A.kdrek2, A.kdrek3, A.kdrek4, A.kdrek5, A.kdrek6, A.Ko_Rkk, 
												SUM(A.nilai_ls) AS nilai
											FROM (									
												SELECT A.Ko_Period AS tahun, B.dt_sesuai AS tgl_bukti, B.Sesuai_No AS no_bukti, B.Sesuai_Ur AS Uraian, LEFT(A.Ko_Rkk,2) AS kdrek1, SUBSTRING(A.Ko_Rkk,4,2) AS kdrek2, SUBSTRING(A.Ko_Rkk,7,2) AS kdrek3, 
												...
												AND ( CAST(B.dt_sesuai AS DATE) <= '".$tgl_2."') 
												AND (A.Ko_unitstr = LEFT('".kd_unit()."',18))
												GROUP BY A.Ko_Period, B.dt_sesuai, B.Sesuai_No, B.Sesuai_Ur, A.Ko_Rkk, A.Ko_DK, A.Ko_sKeg1, A.Ko_sKeg2
											) A
											GROUP BY A.tahun, A.Ko_Rkk, A.Ko_sKeg1, A.Ko_sKeg2, A.kdrek1, A.kdrek2, A.kdrek3, A.kdrek4, A.kdrek5, A.kdrek6, A.no_bukti, A.tgl_bukti, A.uraian
										) a
									)
									SELECT a.kdprogram, a.kdkegiatan, a.kdsubkegiatan, a.nmprogram, a.nmkegiatan, a.nmsubkegiatan, a.Ko_sKeg1, a.Ko_sKeg2 AS kdaktivitas, a.Ur_KegBL1, a.Ur_KegBL2 AS aktivitas,
									b.no_bukti, b.tgl_bukti, b.uraian, a.kdrek1, a.kdrek2, a.kdrek3, a.kdrek4, a.kdrek5, a.kdrek6, rek.Ur_Rk6 AS nmrek6,
									SUM(a.jumlah) AS anggaran, 
									CASE
										WHEN b.kode = 1 THEN  b.nilai
										ELSE 0
									END AS realisasi_gu,
									CASE
										WHEN b.kode = 2 THEN  b.nilai
										ELSE 0
									END AS realisasi_ls
									FROM anggaran a
									LEFT JOIN gabung b ON a.Ko_sKeg1 = b.Ko_sKeg1 AND a.Ko_sKeg2 = b.Ko_sKeg2
									...
									GROUP BY a.kdprogram, a.kdkegiatan, a.kdsubkegiatan, a.nmprogram, a.nmkegiatan, a.nmsubkegiatan, a.Ko_sKeg1, a.Ko_sKeg2, a.Ur_KegBL1, a.Ur_KegBL2, 
									b.no_bukti, b.tgl_bukti, b.uraian, a.kdrek1, a.kdrek2, a.kdrek3, a.kdrek4, a.kdrek5, a.kdrek6, rek.Ur_Rk6
									ORDER BY a.kdprogram, a.kdkegiatan, a.kdsubkegiatan, a.kdrek1, a.kdrek2, a.kdrek3, a.kdrek4, a.kdrek5, a.kdrek6, rek.Ur_Rk6");
		}//tutup if 
		else{
			$rincianSubkegiatan = DB::select("WITH anggaran AS (								
										SELECT bel.Ko_Period AS tahun, bel.Ko_sKeg1, bel.Ko_sKeg2, bel.Ur_KegBL1, bel.Ur_KegBL2, 
									...
									gabung AS (
										SELECT a.* FROM (
											...
											(
												...
												GROUP BY A.Ko_Period, A.No_bp, D.Dt_npd, A.Ur_bprc, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, A.dt_rftrbprc
												UNION ALL
												SELECT A.Ko_Period, B.Dt_Bp AS tgl_bukti, A.No_bp AS no_bukti, B.Ur_bp AS uraian, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, SUM(A.To_Rp) AS nilai
												FROM tb_bprc A INNER JOIN
												tb_bp B ON A.id_bp=B.id_bp
												WHERE ( CAST(B.dt_bp AS DATE) <= ".$tgl_2." ) AND (A.Ko_Period = ".$tahun.") AND (A.Ko_unitstr = LEFT('".kd_unit()."',18))
												AND B.Ko_bp = 2 AND B.Jn_Spm > 2 
												GROUP BY A.Ko_Period, B.Dt_Bp, A.No_bp, B.Ur_bp, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk
											) A
											GROUP BY A.Ko_Period, A.Ko_sKeg1, A.Ko_sKeg2, A.ko_rkk, A.no_bukti, A.tgl_bukti, A.uraian
											UNION ALL
											SELECT 2 AS Kode,
		...
		$data = [
			'rincianSubkegiatan' => $rincianSubkegiatan,
			'ambilbidang' => $bid,
			'ambilskpd' => $skpd,
			'ambilunit' => $unit,
			'refPemda' => $pemda,
