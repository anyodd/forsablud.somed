...
class RptBukuKasPenerimaanBankController extends Controller
{
    ...
    public static function Laporan($tahun, $tgl_1, $tgl_2)
    {
		...
		$bkupdptbank = DB::select("WITH saldoawal_pdpt AS 
						(
						...
						saldoawal_sts as (
							...
							FROM  (
									...
									WHERE  (A.Ko_Period = ".$tahun.")  AND ( CAST(B.dt_byr AS DATE) <  CAST('".$tgl_1."' AS DATE) ) AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) AND A.ko_kas = 1	
						...
						current_pdpt AS (
							...
							AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) 
							AND ( B.Ko_bp IN (1, 42, 43, 11) ) AND A.ko_kas = 1
						),
						current_sts AS (
							...
							INNER JOIN (
								...
								AND (B.Ko_unitstr = LEFT('".kd_unit()."',18)) AND A.ko_kas = 1
