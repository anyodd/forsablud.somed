...
class RptRegisterSppController extends Controller
{
    ...
    public static function Laporan($tahun, $tgl_1, $tgl_2)
    {
		...
		$registerSpp = DB::select("SELECT B.Ko_Period AS tahun, B.Ko_unitstr, B.No_SPi AS spp_no, B.Ko_SPi, B.Dt_SPi AS spp_tgl, B.Ur_SPi AS uraian, 
									CASE B.Ko_SPi
									WHEN 1 THEN 'UP' 
									WHEN 4 THEN 'GU'
									WHEN 2 THEN 'LS'
								...
								tb_spi B ON A.id_spi = B.id
								WHERE B.Ko_SPi <> 5 AND A.Ko_Period = ".$tahun." AND A.Ko_unitstr = LEFT('".kd_unit()."',18) AND CAST(B.Dt_SPi AS DATE) BETWEEN  CAST('".$tgl_1."' AS DATE) AND CAST('".$tgl_2."' AS DATE)
								GROUP BY B.Ko_Period, B.Ko_unitstr, B.No_SPi, B.Dt_SPi, B.Ko_SPi, B.Ur_SPi
								ORDER BY B.Dt_SPi, B.No_SPi");


		$data = [
			'registerSpp' => $registerSpp,
			'ambilbidang' => $bid,
			'ambilskpd' => $skpd,
			'ambilunit' => $unit,
			'refPemda' => $pemda,
