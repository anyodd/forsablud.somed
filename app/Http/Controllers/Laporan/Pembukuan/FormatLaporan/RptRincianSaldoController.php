...
class RptRincianSaldoController extends Controller
{
    ...
    public static function Laporan($tahun, $tgl_1, $tgl_2, $idlevelrekening, $kdrek1, $kdrek2, $kdrek3, $kdrek4, $kdrek5, $kdrek6)
    {
		...
		$rinciansaldobukubesar = DB::select("WITH dataolah AS (
                            SELECT
                                ...
                                a.saldo_normal,
                                CASE
                                    WHEN a.saldo_normal='D' THEN SUM(a.debet)-SUM(a.kredit)
                                ELSE 0 END AS debet,
                                CASE
                                    WHEN a.saldo_normal='K' THEN SUM(a.kredit)-SUM(a.debet)
							...
							GROUP BY a.kdrekgab, a.nmrekgab, a.Ko_unitstr, a.saldo_normal
                        )
						SELECT a.koderekening AS koderekening, a.uraian AS uraianrek,
                            LEFT('".$bid[0]->kode_bidang."',5) AS kodetabel,
                            LEFT('".$bid[0]->kode_bidang."',5) AS kodebuatan,
                            '".$bid[0]->nmbidang."' AS nmtabel,
                            SUM(a.debet) AS debet, SUM(a.kredit) AS kredit
                        FROM dataolah a
                        WHERE (a.debet <> 0 or a.kredit <> 0)
                        GROUP BY a.koderekening, a.uraian
                        UNION ALL
                        SELECT a.koderekening AS koderekening, a.uraian AS uraianrek,
                            LEFT('".$skpd[0]->kode_skpd."',14) AS kodetabel,
                            LEFT('".$skpd[0]->kode_skpd."',14) AS kodebuatan,
                            '".$skpd[0]->uraian_skpd."' AS nmtabel,
                            SUM(a.debet) AS debet, SUM(a.kredit) AS kredit
                        FROM dataolah a
                        WHERE (a.debet <> 0 or a.kredit <> 0)
                        GROUP BY a.koderekening, a.uraian
                        UNION ALL
                        SELECT a.koderekening AS koderekening, a.uraian AS uraianrek,
                            LEFT('".$unit[0]->kode_unit."',18) AS kodetabel,
                            LEFT('".$unit[0]->kode_unit."',18) AS kodebuatan,
                            '".$unit[0]->uraian_unit."' AS nmtabel,
                            SUM(a.debet) AS debet, SUM(a.kredit) AS kredit
                        FROM dataolah a
                        WHERE (a.debet <> 0 or a.kredit <> 0)
                        GROUP BY a.koderekening, a.uraian
                        ORDER BY koderekening, kodebuatan");

		$data = [
			'rinciansaldobukubesar' => $rinciansaldobukubesar,
			'ambilbidang' => $bid,
			'ambilskpd' => $skpd,
			'ambilunit' => $unit,
			'refPemda' => $pemda,
