...
class RealisasiController extends Controller
{
    ...
    {  
        // $data = DB::select("SELECT a.*,SUM(a.To_Rp) piutang,SUM(b.real_rp) realisasi FROM (SELECT a.*, b.id_bprc, b.To_Rp FROM tb_bp a
        // LEFT JOIN tb_bprc b ON a.id_bp = b.id_bp) a
        // LEFT JOIN tb_byr b ON a.id_bprc = b.id_bprc
        // WHERE a.Ko_bp = 1 && a.Ko_Period = '".Tahun()."' && a.Ko_unit1 = '".kd_bidang()."'
        // GROUP BY a.id_bp ORDER BY a.dt_bp DESC");

        $data = DB::select("SELECT * FROM 
        (SELECT a.*,SUM(a.To_Rp) piutang FROM 
        (SELECT a.*, b.id_bprc, b.To_Rp FROM tb_bp a
        LEFT JOIN tb_bprc b ON a.id_bp = b.id_bp) a
        WHERE a.Ko_bp = 1 && a.Ko_Period = '".Tahun()."' && a.Ko_unit1 = '".kd_bidang()."'
        GROUP BY a.id_bp ORDER BY a.dt_bp DESC) a
        LEFT JOIN (SELECT a.id_bprc, SUM(a.real_rp) realisasi FROM tb_byr a WHERE a.Ko_Period = 2023 && a.Ko_unitstr = '".kd_unit()."' GROUP BY a.No_bp) b
        ON a.id_bprc = b.id_bprc");

        $realisasi = Tbbp::where('Ko_bp',1)->orderBy('id_bp')->orderBy('Ko_Period')->get();
        $rincian = Tbbyr::all();
        return view('transaksi.penerimaan.realisasi.index', compact('realisasi','rincian','data'));
    }
    ...
    public function v_bulan(Request $request,$id)
    {
      ...
      $data = DB::select("SELECT * FROM 
                ...
                (SELECT a.*, b.id_bprc, b.To_Rp FROM tb_bp a
                LEFT JOIN tb_bprc b ON a.id_bp = b.id_bp) a
                WHERE a.Ko_bp = 1 && a.Ko_Period = '".Tahun()."' && a.Ko_unit1 = '".kd_bidang()."' && MONTH(a.dt_bp) = '".$bulan."'
                GROUP BY a.id_bp ORDER BY a.dt_bp DESC) a
                LEFT JOIN (SELECT a.id_bprc, SUM(a.real_rp) realisasi FROM tb_byr a WHERE a.Ko_Period = '".Tahun()."' && a.Ko_unitstr = '".kd_unit()."' GROUP BY a.No_bp) b
                ON a.id_bprc = b.id_bprc");
      ...
      return view('transaksi.penerimaan.realisasi.index', compact('realisasi','rincian','data','bulan'));
    }
    ...
    public function tambah($id)
    {
        ...
        $bayar = DB::select("SELECT a.*,a.id_byr,b.id_byr ck FROM tb_byr a 
        LEFT JOIN (SELECT b.id_byr FROM tb_stsrc a 
                JOIN tb_byr b ON a.No_byr = b.No_byr
                WHERE a.Ko_Period = '".Tahun()."' && a.Ko_unitstr ='".kd_unit()."'
                GROUP BY b.id_byr) b
                ON a.id_byr = b.id_byr
        WHERE a.Ko_Period = '".Tahun()."' && a.Ko_unitstr = '".kd_unit()."' && a.No_bp = '".$no_bp->No_bp."'");
        ...
        // if(empty($cekbyr)){
        return view('transaksi.penerimaan.realisasi.create', compact('data','data2','rincian','bank','bayar','pegawai'));
        // }else{
    ...
    public function rincian($id)
    {
        ...
        return view('transaksi.penerimaan.realisasi.rincian', compact( 'realisasi'));
    }

    // public function show($id_byr)
    // {
    //     $rincian = DB::select(DB::raw('select a.*, b.* 
    //         from tb_byr a
    //         join tb_bprc b 
    //         on a.No_bp = b.No_bp
    //         where a.id_byr = '.$id_byr));
    //     return view('transaksi.penerimaan.realisasi.show', compact( 'rincian' ));
    // }
    ...
    public function tbp_pdf($id)
    {
        $tbp = Tbbp::find($id);
        $data = DB::select('CALL SP_TBP("' . $id . '")');
        ...
        $pdf = PDF::loadView('transaksi.penerimaan.realisasi.tbp_pdf', compact('data','jml','pegawai','bendahara'))->setPaper('A4', 'portrait');
        return $pdf->stream('tbp : ' . $tbp->No_bp . '.pdf',  array("Attachment" => false));
    }
