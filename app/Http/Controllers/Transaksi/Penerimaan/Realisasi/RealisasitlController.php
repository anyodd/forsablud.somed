...
class RealisasitlController extends Controller
{
    public function index()
    {
        $data = DB::select(DB::raw('SELECT * FROM tb_bp a
                ...
                GROUP BY a.no_bp, a.Ko_unit1) b ON a.id_bp = b.id_bp
                WHERE a.Ko_bp = 11 && a.Ko_Period = "'.Tahun().'" && a.Ko_unit1 = "'.kd_bidang().'" && b.id_bp IS NOT NULL
                ORDER BY a.dt_bp DESC'));
        $realisasi = Tbbp::where('Ko_bp',11)->orderBy('id_bp')->orderBy('Ko_Period')->get();
        return view('transaksi.penerimaan.realisasi.tl.index', compact('realisasi','data'));
    }
    ...
    public function v_bulan(Request $request,$id)
    {
        ...
        $data = DB::select(DB::raw('SELECT * FROM tb_bp a
                ...
                LEFT JOIN tb_byr b ON a.id_bprc = b.id_bprc
                GROUP BY a.no_bp, a.Ko_unit1) b ON a.id_bp = b.id_bp
                WHERE a.Ko_bp = 11 && a.Ko_Period = "'.Tahun().'" && a.Ko_unit1 = "'.kd_bidang().'" && MONTH(a.dt_bp) = "'.$bulan.'" && b.id_bp IS NOT NULL
                ORDER BY a.dt_bp DESC'));
        $realisasi = Tbbp::where('Ko_bp',11)->orderBy('id_bp')->orderBy('Ko_Period')->get();

        return view('transaksi.penerimaan.realisasi.tl.index', compact('realisasi','data','bulan'));
    }
    ...
    public function tambah($id)
    {
        ...
        // if(empty($cekbyr)){
        return view('transaksi.penerimaan.realisasi.tl.create', compact('data','data2','rincian','bank','bayar','pegawai'));
        // }else{
